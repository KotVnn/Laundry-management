...
export function OrderList({idCus} : {idCus?: string}) {
  ...
  const handleGetData = (type?: string, value?: string, isFromTo?: boolean) => {
    if (query && pageSize) {
      ...
      <Sheet onOpenChange={(open) => handleOpen(open, row)}>
        ...
        <SheetContent className="overflow-y-auto">
          ...
          <div className={`grid grid-cols-2 ${order && order.status && order.status.length && order.status[order.status.length - 1].mID === config.status[config.status.length - 1].mID ? 'md:grid-cols-3' : 'md:grid-cols-4'} items-center justify-baseline p-3 gap-2`}>
            ...
            <div className="inline-block px-4 py-2 bg-teal-800 text-white text-xs font-mono rounded hover:bg-teal-950 cursor-pointer transition">
              <Dialog>
                <DialogTrigger asChild>
                  <p>Print</p>
                </DialogTrigger>
  ...
  const columns: Column<IOrder>[] = [
    {
      title: 'Khách hàng', key: 'customer', render: (val: ICustomer) => <Link href={`/man/order/customer/${val.id}`}>
  ...
  return result && result.data && result.meta && result.data.length > 0 && (
